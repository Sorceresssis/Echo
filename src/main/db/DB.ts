...
class DB {
     ...
     * @param options 建议
     */
    public pragma(source: string, options?: DatabaseConstructor.PragmaOptions | undefined): void {
     ...
     * @returns statement
     */
    public prepare(sql: string): Statement<unknown[]> {
        /**
    ...
    get(sql: string, ...arg: any[]): any {
        return this.db.prepare(sql).get(arg) // prepare()方法会返回一个Statement对象，该对象表示一个准备好的语句
    }
    ...
    public function(name: string, fn: (...arg: any[]) => any) {
     ...
     * @returns 
     */
    public backup(destinationFile: string, options?: DatabaseConstructor.BackupOptions | undefined): Promise<DatabaseConstructor.BackupMetadata> {
