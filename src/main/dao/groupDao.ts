...
export default class GroupDao {
	...
	getGroups(): VO.Group[] {
        ...
        ) SELECT id, name FROM group_list;`) as VO.Group[]
		// 根据组的id获得组下的书籍

		gs.forEach((g) => {
			g.librarys = this.db.all(`
                ...
                ) SELECT id, name FROM library_list;`, g.id, g.id) as VO.LibraryProfile[]
			g.librarys.forEach((l) => {
				l.dataPath = appConfig.getLibraryDirPath(l.id)
			})
		})
		return gs
	}

	public queryLibraryDetail(id: number): VO.LibraryDetail | null {
		const libDetal = this.db.get(`SELECT l.id, l.name, le.use_auxiliary_st AS useAuxiliarySt, le.auxiliary_st AS auxiliarySt, le.intro,
            DATETIME( l.gmt_create, 'localtime' ) AS createTime, DATETIME( l.gmt_modified, 'localtime' ) AS modifiedTime
            FROM library l LEFT JOIN library_extra le ON l.id = le.id WHERE l.id = ?;`, id) as VO.LibraryDetail | null
		if (libDetal) {
			libDetal.useAuxiliarySt = libDetal.useAuxiliarySt ? true : false // 把1,0转换为true, false
		}
		return libDetal
	}
	...
	public updateLibraryExtra(extra: Entity.LibraryExtra): number {
		return this.db.run(`UPDATE library_extra SET auxiliary_st=?, use_auxiliary_st=?, intro=? WHERE id=?;`,
			extra.auxiliarySt, extra.useAuxiliarySt, extra.intro, extra.id).changes
	}
