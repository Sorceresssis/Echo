...
export class config {
    private static configPath: string = path.resolve(path.dirname(app.getPath('exe')), "config.json")

    // readConfig() {
    //     try {
    //         let data = fs.readFileSync(config.configPath, 'utf8')
    //         config = JSON.parse(data)
    //         // 文件存在但是读取错误，重新写入
    //         if (config == null) {
    //             _writeConfig(defaultConfig)
    ...
    // private _writeConfig(data: conf) {
    //     fs.writeFileSync(configPath, JSON.stringify(data), 'utf8', (err: any) => {
    //         console.log(err);
    //     })
    // }
