SELECT type, id, value FROM (
	...
	) AS sore FROM folder WHERE sore > 0 UNION ALL
	SELECT 'series' AS type, id, name AS value, (
		CASE WHEN UPPER(name) LIKE UPPER('%a%') THEN 1 ELSE 0 END +
		CASE WHEN UPPER(name) LIKE UPPER('%b%') THEN 1 ELSE 0 END
	) AS sore FROM series WHERE sore > 0
